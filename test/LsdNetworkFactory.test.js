...
describe("StafiDeposit test", function () {
    before(async function () {
        ...
        this.AccountNetworkAdmin = this.signers[0]
        this.AccountUser1 = this.signers[1]
        this.AccountNode1 = this.signers[2]
        this.AccountSuperNode1 = this.signers[3]
        ...
        this.FactoryDistributor = await ethers.getContractFactory("Distributor", this.AccountNetworkAdmin)
        this.FactoryFeePool = await ethers.getContractFactory("FeePool", this.AccountNetworkAdmin)
        this.FactoryLsdNetworkFactory = await ethers.getContractFactory("LsdNetworkFactory", this.AccountNetworkAdmin)
        this.FactoryLsdToken = await ethers.getContractFactory("LsdToken", this.AccountNetworkAdmin)
        this.FactoryNetworkBalances = await ethers.getContractFactory("NetworkBalances", this.AccountNetworkAdmin)
        this.FactoryNetworkProposal = await ethers.getContractFactory("NetworkProposal", this.AccountNetworkAdmin)
        this.FactoryNodeDeposit = await ethers.getContractFactory("NodeDeposit", this.AccountNetworkAdmin)
        this.FactoryUserDeposit = await ethers.getContractFactory("UserDeposit", this.AccountNetworkAdmin)
        this.FactoryUserWithdraw = await ethers.getContractFactory("StafiWithdraw", this.AccountNetworkAdmin)
    })
    ...
    beforeEach(async function () {
        ...
        await this.ContractStafiNodeManager.connect(this.AccountNetworkAdmin).setNodeTrusted(this.AccountTrustNode1.address, true)
        await this.ContractStafiNodeManager.connect(this.AccountNetworkAdmin).setNodeSuper(this.AccountSuperNode1.address, true)
    ...
    it("super node should deposit/stake success", async function () {
        console.log("latest block: ", await time.latestBlock())
        // enable deposit
        await this.ContractStafiSuperNode.connect(this.AccountNetworkAdmin).setSuperNodeDepositEnabled(true)
        // user deposit
    ...
    it("light node should deposit/stake success", async function () {
        console.log("latest block: ", await time.latestBlock())
        // enable deposit
        await this.ContractStafiLightNode.connect(this.AccountNetworkAdmin).setLightNodeDepositEnabled(true)
        // user deposit
        let userDepositTx = await this.ContractStafiUserDeposit.connect(this.AccountUser1).deposit({ from: this.AccountUser1.address, value: web3.utils.toWei('68', 'ether') })
        let userDepositTxRecipient = await userDepositTx.wait()
        console.log("user deposit tx gas: ", userDepositTxRecipient.gasUsed.toString())
