...
abstract contract StafiVoteBase is StafiBase {
    ...
    function _voteThreshold(uint256 _pId) internal view virtual returns (uint256) {
        return 2 ether;
    }

    function _voteProposal(uint256 _pId, address _voter, bytes32 _proposalId, bool _position) internal returns (bool) {
        string memory contractName = getContractName(_pId, address(this));
        // Get submission keys
        bytes32 proposalNodeKey = keccak256(
            abi.encodePacked(contractName, "proposal.node.key", _pId, _proposalId, _voter)
        );
        bytes32 proposalKey = keccak256(abi.encodePacked(contractName, "proposal.key", _pId, _proposalId));

        bytes32 proposalPositionKey = keccak256(
            abi.encodePacked(contractName, "proposal.position.key", _pId, _proposalId, _position)
        );
