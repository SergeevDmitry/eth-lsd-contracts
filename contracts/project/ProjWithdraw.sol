...
contract ProjWithdraw is StafiBase, IProjWithdraw {
    ...
    function unstake(uint256 _rEthAmount) external onlyLatestContract(pId, "projWithdraw", address(this)) {
        IStafiWithdraw stafiWithdraw = IStafiWithdraw(getContractAddress(1, "stafiWithdraw"));
    ...
    function doWithdraw(address _user, uint256 _amount) external onlyLatestContract(1, "stafiWithdraw", msg.sender) {
        (bool result, ) = _user.call{value: _amount}("");
        require(result, "user failed to withdraw ETH");
    }

    function recycleUserDeposit(uint256 _value) external onlyLatestContract(1, "stafiWithdraw", msg.sender) {
        ProjUserDeposit().recycleDistributorDeposit{value: _value}();
    }

    function doDistributeWithdrawals(uint256 _value) external onlyLatestContract(1, "stafiWithdraw", msg.sender) {
    ...
    ) external onlyLatestContract(pId, "projWithdraw", address(this)) onlySuperUser(pId) {
        IStafiWithdraw stafiWithdraw = IStafiWithdraw(getContractAddress(1, "stafiWithdraw"));
    ...
    function withdrawCommission(uint256 _value) external override onlyLatestContract(1, "stafiWithdraw", msg.sender) {
    ...
    function notifyValidatorExit(
        ...
        IStafiWithdraw stafiWithdraw = IStafiWithdraw(getContractAddress(1, "stafiWithdraw"));
        stafiWithdraw.notifyValidatorExit(msg.sender, _withdrawCycle, _ejectedStartCycle, _validatorIndexList);
    }

    function setWithdrawLimitPerCycle(uint256 _withdrawLimitPerCycle) external onlySuperUser(pId) {
        IStafiWithdraw stafiWithdraw = IStafiWithdraw(getContractAddress(1, "stafiWithdraw"));
        stafiWithdraw.setWithdrawLimitPerCycle(_withdrawLimitPerCycle);
        emit SetWithdrawLimitPerCycle(_withdrawLimitPerCycle);
    }

    function setUserWithdrawLimitPerCycle(uint256 _userWithdrawLimitPerCycle) external onlySuperUser(pId) {
        IStafiWithdraw stafiWithdraw = IStafiWithdraw(getContractAddress(1, "stafiWithdraw"));
