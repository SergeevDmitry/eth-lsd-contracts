...
contract ProjNodeManager is StafiBase, IProjNodeManager {
    ...
    function getTrustedNodeCount() public view override returns (uint256) {
        ...
        return addressSetStorage.getCount(keccak256(abi.encodePacked("nodes.trusted.index", pId)));
    }

    // Get a trusted node address by index
    function getTrustedNodeAt(uint256 _index) public view override returns (address) {
        IAddressSetStorage addressSetStorage = IAddressSetStorage(getContractAddress(1, "addressSetStorage"));
        return addressSetStorage.getItem(keccak256(abi.encodePacked("nodes.trusted.index", pId)), _index);
    }

    // Check whether a node is trusted
    function getNodeTrusted(address _nodeAddress) public view override returns (bool) {
        return getBool(keccak256(abi.encodePacked("node.trusted", pId, _nodeAddress)));
    }
    ...
    function setNodeTrusted(
        ...
        bool _trusted
    ) external override onlyLatestContract(pId, "projNodeManger", address(this)) onlySuperUser(pId) {
    ...
    function getSuperNodeCount() public view override returns (uint256) {
        IAddressSetStorage addressSetStorage = IAddressSetStorage(getContractAddress(1, "addressSetStorage"));
        return addressSetStorage.getCount(keccak256(abi.encodePacked("nodes.super.index", pId)));
    }

    // Get a trusted node address by index
    function getSuperNodeAt(uint256 _index) public view override returns (address) {
        IAddressSetStorage addressSetStorage = IAddressSetStorage(getContractAddress(1, "addressSetStorage"));
        return addressSetStorage.getItem(keccak256(abi.encodePacked("nodes.super.index", pId)), _index);
    }

    // Check whether a node is trusted
    function getSuperNodeExists(address _nodeAddress) public view override returns (bool) {
        return getBool(keccak256(abi.encodePacked("node.super", pId, _nodeAddress)));
    }
